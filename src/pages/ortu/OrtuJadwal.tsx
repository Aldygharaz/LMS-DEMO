...
function ChildScheduleView({
  ...
  const holidaysQuery = trpc.ortu.listHariLibur.useQuery();
  ...
  if (!dashboard.data) {
    return (
      <div className="py-12 text-center text-sm text-muted-foreground">
        Memuat data jadwal ananda...
      </div>
    );
  }
  const data = dashboard.data;
  const jadwalList = data.jadwalList;
  ...
  const exportJadwalCSV = () => {
    ...
    const headers = ["No", "Nama Siswa", "Hari", "Jam Mulai", "Jam Selesai", "Mata Pelajaran", "Guru Pengampu"];
    ...
    exportToCSV(`Jadwal_Pelajaran_${childName.replace(/\s+/g, "_")}`, headers, rows);
    toast.success(`Jadwal pelajaran ananda ${childName} berhasil diekspor!`);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="p-5 rounded-2xl bg-card border border-border shadow-lg space-y-3">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-3 border-b border-border/50">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-base font-bold font-brand text-foreground">
              Jadwal Belajar Hari Ini ({today})
            </h3>
          </div>

          <div className="flex items-center gap-3">
            <span className="text-xs font-bold text-green-600 dark:text-[#57F287]">
              {jadwalHariIni.length} Sesi Aktif
            </span>
            <Button
              type="button"
              variant="outline"
              onClick={exportJadwalCSV}
              className="h-8 bg-background border-border text-foreground hover:bg-secondary text-xs font-semibold rounded-xl print:hidden"
            >
          ...
          {jadwalHariIni.map((j) => (
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <span className="font-mono text-xs text-green-600 dark:text-[#57F287] bg-green-100 dark:bg-[#23A559]/10 px-2 py-0.5 rounded font-bold">
      ...
      {/* Tabs Filter Bar */}
      <div className="flex items-center gap-2 p-1.5 bg-card rounded-2xl border border-border print:hidden">
        <button
          type="button"
        ...
        >
          ...
          Sesi Pengganti Ananda ({makeUpClassesQuery.data?.length ?? 0})
        </button>
      ...
      {activeTab === "reguler" && (
        <Card className="bg-card border-border shadow-xl rounded-2xl overflow-hidden">
              ...
              <div className="flex items-center gap-2">
                ...
                <div>
                  <CardTitle className="text-base font-bold font-brand text-foreground">
                    Jadwal Mingguan Lengkap ({childName})
                  </CardTitle>
          ...
          <CardContent className="p-5 space-y-4">
            {jadwalList.length === 0 && (
              <div className="py-12 text-center text-sm text-muted-foreground">
                Belum ada jadwal pelajaran yang ditentukan untuk kelas ananda.
              </div>
            )}
      ...
      {activeTab === "pengganti" && (
        <div className="space-y-6">
          <div className="p-4 rounded-2xl bg-card border border-border shadow-lg flex items-center gap-3">
            ...
            <div>
              <span className="text-xs font-bold text-foreground block">
                Pemberitahuan Sesi Kelas Pengganti Ananda
              </span>
              <p className="text-[11px] text-muted-foreground mt-0.5">
                Sesi tatap muka tambahan yang diselenggarakan guru pengampu untuk menggantikan jam pelajaran ananda yang terbentur hari libur.
              </p>
            </div>
          </div>
          ...
          <Card className="bg-card border-border shadow-xl rounded-2xl overflow-hidden">
            <CardHeader className="border-b border-border/50 pb-4">
              <CardTitle className="text-base font-bold font-brand text-foreground">
                Daftar Sesi Pengganti Kelas Ananda ({childName})
              </CardTitle>
            </CardHeader>
                  ...
                  {makeUpClassesQuery.data?.length === 0 && (
                    <TableRow>
                      <TableCell colSpan={5} className="text-center py-10 text-xs text-muted-foreground">
                        Tidak ada sesi kelas pengganti yang dijadwalkan untuk kelas ananda saat ini.
                      </TableCell>
                    </TableRow>
                  )}
...
export default function OrtuJadwal() {
  ...
  return (
    <SchoolLayout role="orang_tua" title="Jadwal Pelajaran Anak" nav={ORTU_NAV}>
      {/* Header Banner */}
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-card p-5 rounded-2xl border border-border shadow-lg print:hidden">
        <div>
          <h2 className="text-xl font-bold font-brand text-foreground">
            Jadwal Pelajaran Sekolah Anak
          </h2>
          <p className="text-xs text-muted-foreground mt-0.5">
            Membantu orang tua mempersiapkan buku pelajaran dan perlengkapan harian anak
          </p>
        </div>
